...
class PromptManager:
    ...
    def __init__(self, prompts_dir: Optional[Path] = None):
        ...
        Args:
            prompts_dir: Root directory for prompts. Defaults to ./prompts/
        """
        if prompts_dir is None:
            # Default to prompts/ relative to this file's parent directory
            # For mech-util, prompts are typically in the agent repos, not here
            # So we'll require explicit prompts_dir or PROMPTS_DIR env var
            env_override = os.environ.get("PROMPTS_DIR")
            if env_override:
                prompts_dir = Path(env_override)
            else:
                # Default fallback - but this should be set explicitly
        ...
        env_override = os.environ.get("PROMPTS_DIR")
        if env_override:
            self.prompts_dir = Path(env_override)
